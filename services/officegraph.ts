import { https, httpsGetAll } from '@/contexts/httphelper';
...
export async function getMemberOfs(accessToken: string) {
	const items = await httpsGetAll<TransitiveMemberOf>(
		accessToken,
...
export async function getSiteCollection(accessToken: string, sharePointTenantName: string, sitePath: string) {
	return https<SiteCollection>(
		accessToken ?? '',
		'GET',
		`https://graph.microsoft.com/v1.0/sites/${sharePointTenantName}.sharepoint.com:/${sitePath}`,
	);
}
